...
var name_to_builtin_func = {
    ...
    'scalar_subtraction': {
        ...
        func: function () {
            ...
            if (!(0, datatype_def_1.is_number)(args[0]) || !(0, datatype_def_1.is_number)(args[1]) || args.length !== 2)
                throw new error_def_1.FuncArgError("Built in function 1 SCALAR_SUBTRACTION accepts parameters: NUMBER, NUMBER. Received: ".concat(args[0], ", ").concat(args[1]));
            return Number(args[0]) - Number(args[1]);
        }
    },
    //multiplication
    'scalar_multiplication': {
        ...
        func: function () {
            ...
            if (!(0, datatype_def_1.is_number)(args[0]) || !(0, datatype_def_1.is_number)(args[1]) || args.length !== 2)
                throw new error_def_1.FuncArgError("Built in function 2 SCALAR_MULTIPLICATION accepts parameters: NUMBER, NUMBER. Received: ".concat(args[0], ", ").concat(args[1]));
            return Number(args[0]) * Number(args[1]);
        }
    },
    //division
    'scalar_division': {
        ...
        func: function () {
            ...
            if (!(0, datatype_def_1.is_number)(args[0]) || !(0, datatype_def_1.is_number)(args[1]) || args.length !== 2)
                throw new error_def_1.FuncArgError("Built in function 3 SCALAR_DIVISION accepts parameters: NUMBER, NUMBER. Received: ".concat(args[0], ", ").concat(args[1]));
            return Number(args[0]) / Number(args[1]);
        }
    },
    /**
    ...
    'scalar_to_function_points': {
        ...
        func: function () {
            ...
            if (!(0, datatype_def_1.is_number)(args[0]) || !(0, datatype_def_1.is_integer)(args[1]) || args.length !== 2)
                throw new error_def_1.FuncArgError("Built in function 5 SCALAR_TO_FUNCTION_POINTS accepts parameters: NUMBER, INTEGER. Received: ".concat(args[0], ", ").concat(args[1]));
            var val = Number(args[0]);
            var length = Number(args[1]);
            /**
             * Converts a number to an array of length 2 tuples
    ...
    'apply_interest_rate': {
        ...
        func: function () {
            ...
            if (!datatype_def_1.declared_type_verifier[0](args[0]) || !datatype_def_1.declared_type_verifier[1](args[1]))
                throw new error_def_1.FuncArgError("Built in function 6 APPLY_INTEREST_RATE accepts parameters: NUMBER, FUNCTION_POINT_SERIES. Received: ".concat(args[0], ", ").concat(args[1]));
            var rate = args[0];
            var ser = args[1];
            for (var i = 1; i < ser.length; i++) {
                ser[i][1] = (1 + rate) * ser[i - 1][1];
